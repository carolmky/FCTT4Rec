...
class MeantimeModel(BertBaseModel):
    def __init__(self, args,train_load):
        ...
        ##### Footers
        # Token Embeddings
        self.token_embedding = TokenEmbedding(args,train_load)
        # Absolute Embeddings
        ...
        if absolute_kernel_types is not None and len(absolute_kernel_types) > 0:
            for kernel_type in absolute_kernel_types.split('-'):
                ...
                elif kernel_type == 't':  # time difference
                    emb = TimeEmbedding(args)
        ...
        if relative_kernel_types is not None and len(relative_kernel_types) > 0:
            for kernel_type in relative_kernel_types.split('-'):
                if kernel_type == 't':  # time difference
                    emb = TimeEmbedding(args)
                elif kernel_type == 'e':
        ...
        if args.headtype == 'dot':
            self.head = BertDotProductPredictionHead(args, self.token_embedding.embedding_item,self.token_embedding.embedding_user)
        ...
        self.n_layers = 3
        self.Graph = train_load.dataset.Graph
    ...
    def get_logits(self, d,batch_idx,emb_user,emb_item):
        x = d['tokens']
        attn_mask = (x > 0).unsqueeze(1).repeat(1, x.size(1), 1).unsqueeze(1)  # B x 1 x T x T
        token_embeddings = self.dropout(self.token_embedding(d,batch_idx,emb_user,emb_item)) # B x T x H
    ...
    def get_scores(self, d, logits,uid,last_hidden):
        # logits : B x H or M x H

        if self.training:  # logits : M x H, returns M x V
            h = self.head(logits,uid,last_hidden)  # M x V   2926m
        else:  # logits : B x H,  returns B x C
            candidates = d['candidates']  # B x C
            h = self.head(logits, uid,last_hidden,candidates)  # B x C
        return h
