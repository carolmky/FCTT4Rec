from FCTT.loggers import *
# from config import STATE_DICT_KEY, OPTIMIZER_STATE_DICT_KEY, TRAIN_LOADER_RNG_STATE_DICT_KEY
from FCTT.config import *
from FCTT.utils import AverageMeterSet
from FCTT.utils import fix_random_seed_as
from FCTT.analyze_table import find_saturation_point
...
class AbstractTrainer(metaclass=ABCMeta):
    ...
    def train_one_epoch(self, epoch, accum_iter, train_loader, **kwargs):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            if self.pilot and batch_idx >= self.pilot_batch_cnt:
                ...
                break
            if self.is_gcn==True:
                emb_user,emb_item = self.gcn(batch,batch_idx,self.is_gcn)
            ...
            loss = self.calculate_loss(batch,batch_idx,self.is_gcn)
            # torch.cuda.empty_cache()
            if isinstance(loss, tuple):
                loss, extra_info = loss
                for k, v in extra_info.items():
                    average_meter_set.update(k, v)
            loss.backward(retain_graph=True) #4938
            # loss.backward()
            torch.cuda.empty_cache()#2956
    ...
    def validate(self, epoch, accum_iter, mode, doLog=True, **kwargs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                metrics = self.calculate_metrics(batch,batch_idx,self.is_gcn)
    ...
    def gcn(self,batch,batch_idx,is_gcn):
        print('1111')
        a = self.is_gcn()
